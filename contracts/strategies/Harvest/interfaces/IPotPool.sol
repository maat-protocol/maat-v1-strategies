...
interface IPotPool {
    ...
    function periodFinishForToken(
        address _rewardToken
    ) external view returns (uint);

    function rewardRateForToken(
        address _rewardToken
    ) external view returns (uint);

    function lastUpdateTimeForToken(
        address _rewardToken
    ) external view returns (uint);

    function rewardPerTokenStoredForToken(
        address _rewardToken
    ) external view returns (uint);

    function userRewardPerTokenPaidForToken(
        address _rewardToken,
        address _user
    ) external view returns (uint);
